    ...
    class ClassUsuario_Per
    {
        ...
        public bool InserirUsuario(ClassUsuario_Dom usuar_Dom)
        {
            ...
            try
            {
                ...
                if (usuario_Dom.Foto != null)
                {
                    ...
                    sqlCommand.CommandText = "INSERT INTO Usuarios (nome, dt_nascimento, tel, cel, email, cep, logradouro, bairro, cidade, uf,  identificador, senha, foto) VALUES (@nome, @dt_nascimento, @tel, @cel, @email, @cep, @logradouro, @bairro, @cidade, @uf, @identificador, '" + crypt.Encrypt(usuario_Dom.Senha) + "', @foto)";
                }  
                else
                {
                    sqlCommand.CommandText = "INSERT INTO Usuarios (nome, dt_nascimento, tel, cel, email, cep, logradouro, bairro, cidade, uf,  identificador, senha) VALUES (@nome, @dt_nascimento, @tel, @cel, @email, @cep, @logradouro, @bairro, @cidade, @uf, @identificador, '" + crypt.Encrypt(usuario_Dom.Senha) + "')";
                }
        ...
        public bool Login(ClassUsuario_Dom usuar_Dom)
        {
            ...
            try
            {
                ...
                //usuario_Dom.Situacao = usuarios.Rows[0]["situacao"].ToString();
                //usuario_Dom.Autenticacao = usuarios.Rows[0]["autenticacao"].ToString();
                return true;
            }
            catch (Exception error)
            {
